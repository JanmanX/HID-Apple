...
static unsigned int swap_opt_cmd = 1;
...
// Jan
static unsigned int swap_fn_ctrl = 1;
module_param(swap_fn_ctrl, uint, 0644);
MODULE_PARM_DESC(swap_fn_ctrl, "Swap the FN and left CTRL keys. "
								"[0] = as-is, Mac layout. 1 = swapped");

static unsigned int use_ejectcd_as_delete = 1;
module_param(use_ejectcd_as_delete, uint, 0644);
MODULE_PARM_DESC(use_ejectcd_as_delete, "Use ejectcd key as delete ([0] = disabled, "
		"1 = ejectcd as del)");
...
static const struct apple_key_translation apple_fn_keys[] = {
		...
		{ KEY_LEFT,	KEY_HOME },
		{ KEY_RIGHT,	KEY_END },
		{ 84, 		KEY_LEFTCTRL, 	    APPLE_FLAG_SKEY},
		{ KEY_EJECTCD,	KEY_DELETE,         APPLE_FLAG_SKEY},
		{ }
};
...
static const struct apple_key_translation swapped_fn_ctrl_keys[] = {
		{ KEY_LEFTCTRL, KEY_FN },
		{ KEY_FN, KEY_LEFTCTRL },
		{ }
};
		...
		unsigned long **bit, int *max)
{
	...
	if (usage->hid == (HID_UP_CUSTOM | 0x0003)) {
		...
		hid_map_usage_clear(hi, usage, bit, max, EV_KEY, fnkeycode);
